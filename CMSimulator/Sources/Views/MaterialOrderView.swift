...
struct MaterialOrderView: View {
    ...
    init(request: MaterialOrderRequest, spendingPower: Double,
        ...
        _quantity = State(initialValue: max(1, request.suggestedQuantity))
    }

    private var maxQuantity: Double { max(1, request.suggestedQuantity * 2).rounded() }
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        HStack {
                            ...
                            Button(String(localized: "Use", comment: "Button to apply the suggested order quantity")) {
                                quantity = max(1, request.suggestedQuantity)
                            }
                            .font(.caption.bold())
                            .buttonStyle(.plain)
