...
struct VendorBidPickerView: View {
    ...
    private func statLine(_ bid: VendorBid) -> some View {
        HStack(spacing: 12) {
            statBadge("Cost", bid.costFactor, isBest: bid.costFactor == bestCost)
            statBadge("Relationship", bid.alliedDiscountFactor, isBest: bid.alliedDiscountFactor == bestDiscount)
            statBadge("Quality", bid.qualityFactor, isBest: bid.qualityFactor == bestQuality)
        }
